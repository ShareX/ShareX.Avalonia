...
description: How to write effective XerahS Improvement Proposals (XIPs) for new features, architecture changes, and cross-platform implementations. Use for XIP structure, templates, and patterns. For creating/syncing XIPs use the xip-sync skill.
---
...
**Workflow**: **GitHub is the source of truth** for XIPs; the `tasks/` folder is a **backup**. Create and edit XIPs as GitHub issues (label `xip`), then run the sync script to update the tasks folder. See [xip-sync](.ai/skills/xip-sync/SKILL.md) for create/edit/sync workflows.
...
Every XIP lives as a **GitHub issue** (label `xip`). The issue **body is the full XIP content**. The `tasks/` folder is a backup generated by the xip-sync script.
...
### Create / Edit / Sync
Use the **[xip-sync](.ai/skills/xip-sync/SKILL.md)** skill to create issues, edit them, and run `sync-from-github.ps1` to refresh the tasks backup.
...
The **tasks folder is a backup** of GitHub. Sync with `.ai/skills/xip-sync/scripts/sync-from-github.ps1`. Status is not reflected in paths; all backup files live in **`tasks/`** as `XIP####-Title-Slug.md`.
