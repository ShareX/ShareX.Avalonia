...
**Scope:** ShareX.ImageEditor (ImageEditor) — adjustment/filter effects and their application pipeline. Host (XerahS) provides optional GRContext when applying effects.
...
- **Constraint:** The context is valid only on the **render/UI thread** and only while the lease/scope is held. Effect application that wants GPU must run on that thread (or dispatch to it) and complete within the scope.
...
- [ ] Host (XerahS/EditorView) gets Avalonia’s `GRContext` and provides it to the effect pipeline when applying effects (e.g. on UI thread).
...
- [Avalonia Skia – TryGetGrContext](https://api-docs.avaloniaui.net/docs/M_Avalonia_Skia_ISkiaGpuWithPlatformGraphicsContext_TryGetGrContext)
- [SkiaSharp GRContext](https://learn.microsoft.com/en-us/dotnet/api/skiasharp.grcontext)
- ImageEditor: `ImageEditor/src/ShareX.ImageEditor/Core/ImageEffects/Adjustments/ImageEffect.cs` (ApplyColorFilter, ApplyColorMatrix, ApplyPixelOperation)
