...
- **XerahS** ΓÇö Avalonia desktop app; embeds EditorView and can obtain `GRContext` from the Avalonia Skia renderer.
...
## Current State (as of Feb 2026 ΓÇö second audit)
...
| Area | Status |
|---|---|
| **Phase 1** ΓÇö GRContext / GPU surface path | Γ¥î NOT STARTED |
| **Phase 2a** ΓÇö BlackAndWhite via color matrix | Γ¥î NOT DONE |
| **┬º3.2** ΓÇö `ColorizeImageEffect` refactor | Γ¥î NOT DONE |
| **┬º3.5** ΓÇö `SKFilterQuality` deprecation | Γ¥î NOT DONE |
| **┬º3.6** ΓÇö Redundant `Category` overrides in 7 filter effects | Γ¥î NOT DONE |
| **┬º3.8** ΓÇö `new Random()` per call in Slice/TornEdge | Γ¥î NOT DONE |
| Phase 3.1 (GPU threshold) | Γ¥î Pending Phase 1 |
| ┬º3.3 GammaImageEffect LUT caching | ΓÜá∩╕Å Low priority |
| ┬º3.4 BlurImageEffect allocation reduction | ΓÜá∩╕Å Low priority |
| ┬º3.7 SelectiveColor `ToHsl` short-circuit | ΓÜá∩╕Å Profile first |
...
| Gamma | `ApplyColorFilter` (table LUT) | After Phase 1 | Rebuilds LUT every call ΓÇö ┬º3.3 |
| Colorize | Custom canvas (bypasses helper) | After ┬º3.2 + Phase 1 | ┬º3.2 |
| **BlackAndWhite** | `ApplyPixelOperation` | Γ¥î until Phase 2a | ┬º2a |
| **ReplaceColor** | `ApplyPixelOperation` (unsafe Γ£à) | CPU only | Per-pixel; no matrix equivalent |
| **SelectiveColor** | `ApplyPixelOperation` (unsafe Γ£à) | CPU only | Per-pixel HSL; ┬º3.7 |
| Blur | `SKImageFilter.CreateBlur` | N/A | 3-bitmap chain ΓÇö ┬º3.4 |
| Sharpen | `SKImageFilter.CreateMatrixConvolution` | N/A | Clean |
| Pixelate | Downscale/upscale | N/A | Deprecated `SKFilterQuality` ΓÇö ┬º3.5 |
| Border | Canvas drawing | N/A | Redundant `Category` override ΓÇö ┬º3.6 |
| Glow Γ£à | `SKColorFilter` + blur; asymmetric resize | N/A | Redundant `Category` override ΓÇö ┬º3.6 |
| Reflection Γ£à | Canvas + gradient; correct flip + skew width | N/A | Redundant `Category` override ΓÇö ┬º3.6 |
| Shadow Γ£à | `SKColorFilter` + blur; asymmetric resize | N/A | Redundant `Category` override ΓÇö ┬º3.6 |
| Outline Γ£à | `SKImageFilter.CreateDilate` + DstOut | N/A | Redundant `Category` override ΓÇö ┬º3.6 |
| TornEdge | Path generation | N/A | `new Random()` per call ΓÇö ┬º3.6, ┬º3.8 |
| Slice Γ£à | Canvas drawing; robust bounds | N/A | `new Random()` per call ΓÇö ┬º3.6, ┬º3.8 |
| Resize | `SKBitmap.Resize` | N/A | Deprecated `SKFilterQuality` ΓÇö ┬º3.5 |
...
## 1. Phase 1: Use GPU for color-matrix / color-filter effects Γ¥î NOT STARTED

### 1.1 Obtain GRContext from Avalonia (host-specific)

- **XerahS:** In the host (e.g. `EditorView` or wherever effects are triggered), get the current Skia GPU context from the Avalonia renderer: `IPlatformGraphics` (e.g. via `AvaloniaLocator` or dependency) ΓåÆ cast to `ISkiaGpuWithPlatformGraphicsContext` ΓåÆ `TryGetGrContext()` (returns `IScopedResource<GRContext>?`). Or use `ISkiaSharpApiLease` / `GrContext` if available during rendering.
...
- **GPU threshold:** For small images (e.g. `width*height < 160_000`, roughly Γëñ 400├ù400 px) the GPU read-back overhead can dominate. Consider applying GPU only above a pixel-count threshold (see ┬º3.1).
...
- **`ColorizeImageEffect` note:** This effect currently manages its own canvas instead of using the `ApplyColorFilter` helper (see ┬º3.2). It must be refactored to use the helper so it benefits from the GPU path too.
...
### 2a. BlackAndWhite ΓåÆ color matrix Γ¥î NOT DONE
...
**Problem:** Uses `ApplyPixelOperation` so it cannot use the GPU path even after Phase 1 is implemented. Also note it discards source alpha (replaces every pixel with fully-opaque White or Black ΓÇö intentional behaviour, not a bug, but document it).
...
### 2b. ReplaceColor and SelectiveColor: fast pixel loop Γ£à DONE (by Jaex, Round 1)
...
### 3.1 GPU read-back threshold (after Phase 1) Γ¥î NOT DONE
...
### 3.2 `ColorizeImageEffect` ΓÇö refactor to use helper Γ¥î NOT DONE
...
### 3.3 `GammaImageEffect` ΓÇö optional LUT caching ΓÜá∩╕Å LOW PRIORITY
...
### 3.4 `BlurImageEffect` ΓÇö intermediate bitmap reduction ΓÜá∩╕Å LOW PRIORITY
...
### 3.5 `SKFilterQuality` deprecation Γ¥î NOT DONE

SkiaSharp ΓëÑ 2.88 deprecated `SKFilterQuality` in favour of `SKSamplingOptions`. Two call sites remain:
...
### 3.6 Redundant `Category` overrides in Filter effects Γ¥î NOT DONE
...
### 3.7 `SelectiveColorImageEffect` ΓÇö per-pixel `ToHsl` cost ΓÜá∩╕Å PROFILE FIRST
...
### 3.8 `SliceImageEffect` and `TornEdgeImageEffect` ΓÇö `new Random()` per call Γ¥î NOT DONE
...
|---|---|---|---|
| **2b** | `ApplyPixelOperation` unsafe pointer loop | Γ£à **DONE** (Round 1) | ReplaceColor/SelectiveColor at native speed. |
...
| **2a** | BlackAndWhite via two-pass color filter | Γ¥î Pending | No per-pixel loop; GPU-eligible after Phase 1. |
| **3.8** | `Random.Shared` in Slice/TornEdge | Γ¥î Pending (quick fix) | No determinism issue on rapid calls; no allocation. |
| **3.5** | `SKFilterQuality` ΓåÆ `SKSamplingOptions` | Γ¥î Pending (quick fix) | Removes deprecated-API warnings. |
| **3.6** | Remove redundant `Category` overrides (7 files) | Γ¥î Pending (quick fix) | Code consistency. |
| **3.2** | `ColorizeImageEffect` refactor to use helper | Γ¥î Pending (prerequisite for GPU) | Colorize gains GPU path. |
| **1** | GPU surface via `GRContext` for `ApplyColorFilter`/`ApplyColorMatrix` | Γ¥î Pending | All matrix/filter effects on GPU when context available. |
| **3.1** | GPU read-back threshold | Γ¥î Pending (part of Phase 1) | Skip GPU for tiny images. |
| **3.3** | `GammaImageEffect` LUT caching | ΓÜá∩╕Å Low priority | Minor live-preview speedup. |
| **3.4** | `BlurImageEffect` allocation reduction | ΓÜá∩╕Å Low priority | One fewer SKBitmap per blur call. |
| **3.7** | `SelectiveColor` `ToHsl` short-circuit | ΓÜá∩╕Å Profile first | Potential speedup at 4K scale. |

**Implementation order recommendation:**
1. **3.8** ΓÇö two lines changed, no risk, solves determinism bug.
2. **3.5** ΓÇö trivial, no risk, removes warnings.
3. **3.6** ΓÇö trivial, no risk, 7 one-line deletions.
4. **2a** ΓÇö straightforward, no GPU dependency.
5. **3.2** ΓÇö prerequisite for Phase 1 GPU to cover Colorize.
6. **1 + 3.1** ΓÇö main GPU work: wire GRContext, size threshold.
7. **3.3, 3.4, 3.7** ΓÇö only if profiling shows need.
...
- BlackAndWhite (┬º2a): [ImageEditor/src/ShareX.ImageEditor/Core/ImageEffects/Adjustments/BlackAndWhiteImageEffect.cs](../ImageEditor/src/ShareX.ImageEditor/Core/ImageEffects/Adjustments/BlackAndWhiteImageEffect.cs)
- Colorize (┬º3.2): [ImageEditor/src/ShareX.ImageEditor/Core/ImageEffects/Adjustments/ColorizeImageEffect.cs](../ImageEditor/src/ShareX.ImageEditor/Core/ImageEffects/Adjustments/ColorizeImageEffect.cs)
- Pixelate (┬º3.5): [ImageEditor/src/ShareX.ImageEditor/Core/ImageEffects/Filters/PixelateImageEffect.cs](../ImageEditor/src/ShareX.ImageEditor/Core/ImageEffects/Filters/PixelateImageEffect.cs)
- Resize (┬º3.5): [ImageEditor/src/ShareX.ImageEditor/Core/ImageEffects/Manipulations/ResizeImageEffect.cs](../ImageEditor/src/ShareX.ImageEditor/Core/ImageEffects/Manipulations/ResizeImageEffect.cs)
- Slice (┬º3.8): [ImageEditor/src/ShareX.ImageEditor/Core/ImageEffects/Filters/SliceImageEffect.cs](../ImageEditor/src/ShareX.ImageEditor/Core/ImageEffects/Filters/SliceImageEffect.cs)
- TornEdge (┬º3.8): [ImageEditor/src/ShareX.ImageEditor/Core/ImageEffects/Filters/TornEdgeImageEffect.cs](../ImageEditor/src/ShareX.ImageEditor/Core/ImageEffects/Filters/TornEdgeImageEffect.cs)
