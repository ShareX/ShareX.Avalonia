# XIP0041: Native AOT – Full Implementation (Issue #149)

## Summary

**Goal**: Fully implement [GitHub Issue #149](https://github.com/ShareX/XerahS/issues/149) — *[Feature]: Consider Enabling NAOT to Reduce Installer Size* — by enabling .NET **Native AOT** for the XerahS host so the Windows installer is smaller (~90 MB today; target 50–87% reduction), with faster startup and no separate .NET runtime.
...
### 2.1 System.Text.Json migration (AOT‑friendly JSON)
...
| **Phase 3** | XerahS.History, XerahS.Indexer, XerahS.Media – all JsonConvert → JsonSerializer; Newtonsoft removed from csproj | Done |
...
| **Phase 5** | PluginTemplate (MyProvider + csproj), XerahS.Mobile.Core (MobileAmazonS3ConfigViewModel) – STJ/JsonNode | Done |
...
Plugins and most core JSON paths no longer use Newtonsoft; the main AOT blocker from the architecture doc (§7.2) is largely addressed.

### 2.2 Reflection removed: MainWindow → EditorView
...
|------------------------|--------|
| Newtonsoft in plugins / most core JSON | Addressed (STJ migration Phases 1–5) |
| MainWindow → EditorView reflection | **Fixed** (direct call) |
| ViewLocator reflection | **Mitigated** – TrimmerRoots.xml preserves Views/ViewModels |
| CustomUploaderFunction discovery | **Mitigated** – TrimmerRoots.xml preserves all function types |
| x:CompileBindings | **Done** – App.axaml has `x:CompileBindings="True"` |
| PublishAot + packaging | **Done** – csproj conditional + package-windows.ps1 `-EnableNativeAot` |
...
### Phase 1: Enable AOT and pass smoke test ✅ (infrastructure done; smoke test pending)

**1.1 Enable AOT behind a property** ✅ DONE
...
**1.3 XAML: compiled bindings** ✅ DONE (partial)
...
**1.4 First AOT publish and packaging** ✅ DONE
...
**1.5 Smoke test** ✅ SCRIPT ADDED

- **Script**: `build/scripts/smoke-test-aot.ps1` — runs AOT publish for win-x64 (or `-RuntimeIdentifier win-arm64`). ILCompiler is auto-restored; no extra workload required.
...
- AOT publish produces a single native exe (or small set of outputs) for win-x64. ✅ (run script or package-windows.ps1 -EnableNativeAot)
- Installer builds from AOT output and Plugins folder. ✅
- Smoke test passes without runtime trim/reflection errors. ⏳ (run manually after publish)

**1.6 Windows ARM64 (Native AOT on ARM64 machines)** ✅ DONE
...
- **vcvarsall limitation**: On ARM64 hosts, `vcvarsall.bat arm64` does **not** add the `Hostarm64\arm64` tools directory to `PATH`, so the ILCompiler’s `findvcvarsall.bat` fails with “Platform linker not found.”
...
- **Result**: AOT publish for `win-arm64` no longer fails with “Platform linker not found” on ARM64 machines. Remaining failures (if any) are trim/AOT analysis in app or dependencies (e.g. WinForms, reflection), not tooling.

**1.7 WinForms and AOT** ✅ DONE (Phase 1: suppressions)
...
- **Phase 1 (current)**: WinForms remains enabled. IL3000 is suppressed when `PublishAot=true` so ILC does not fail on WinForms’ `Assembly.Location`:
  - **XerahS.App.csproj**: `NoWarn` includes `IL3000` when `PublishAot=true`.
  - **XerahS.WatchFolder.Daemon.csproj**: same `NoWarn` for `IL3000` when `PublishAot=true` (the daemon is published AOT as part of the Windows package and references **XerahS.Platform.Windows** → WinForms).
...
- See **§4 Risks** (WinForms under AOT).

**1.8 AOT UI not clickable (menus, controls)** ⏳ OPEN (blocker for AOT UX)
...
### Phase 2: Optional – reduce trimmer roots (smaller binary)

**2.1 ViewLocator**

- Replace `Type.GetType(name)` + `Activator.CreateInstance(type)` with a **compile-time map** (manual registration or source generator) from ViewModel type → View type.
...
- If load/save of settings or uploader config fails under AOT despite trimmer roots:
  ...
  - See SYSTEM_TEXT_JSON_MIGRATION.md §2.3 and §3 Phase 2.
...
**3.3 Third-party validation** ✅ DOCUMENTED

- **FluentAvalonia**: Test under AOT; add trimmer roots or fixes if font/icon loading fails. Documented in `docs/architecture/NATIVE_AOT_IMPLEMENTATION_ISSUE_149.md` §9.
- **ImageEditor (submodule)**: Audit for reflection/dynamic loading; add trimmer roots or fixes if needed under AOT. Same doc §9.
...
- FluentAvalonia/ImageEditor: validation documented (§3.3); add trimmer roots if issues appear.
...
| **WinForms under AOT** | System.Windows.Forms is not AOT-compatible (IL3000: Assembly.Location; IL2104/IL3053 in dependencies). **Phase 1**: `NoWarn` for IL3000 in **XerahS.App** and **XerahS.WatchFolder.Daemon** when `PublishAot=true`. **Phase 2 (optional)**: AOT-safe screen/clipboard so host/daemon do not reference WinForms. See §1.7. |
| **AOT UI not clickable** | Under AOT on Windows (x64 and ARM64), only window chrome works; menus and controls do not respond. TrimmerRootAssembly for Avalonia/FluentAvalonia was added but **outcome unchanged**; cause likely beyond trimmer (e.g. Avalonia Win32 input under AOT). **Status**: open blocker. See §1.8. |
